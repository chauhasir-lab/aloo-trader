...
const s = {
  ...
  navItem: (active) => ({ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", padding: "10px 2px", gap: 2, background: "none", border: "none", color: active ? clr.accent : clr.muted, fontSize: "12px", cursor: "pointer" }),
...
const Modal = ({ open, onClose, title, children }) => !open ? null : (
  <div style={{ position: "fixed", inset: 0, background: "#000c", zIndex: 1000, display: "flex", alignItems: "flex-end" }}>
    <div style={{ background: clr.card, borderRadius: "20px 20px 0 0", width: "100%", maxWidth: 480, maxHeight: "92vh", display: "flex", flexDirection: "column", border: `1px solid ${clr.border}` }}>
      <div style={{ ...s.rowBetween, padding: 14, borderBottom: `1px solid ${clr.border}` }}>
        <span style={{ fontWeight: 700, fontSize: 16 }}>{title}</span>
        <button onClick={onClose} style={s.btnSm()}><Icon name="x" size={14} /></button>
      </div>
      <div style={{ overflowY: "auto", padding: "14px" }}>{children}</div>
    </div>
  </div>
);
...
const DispatchAnalyticsScreen = ({ dispatches = [], parties = [], coldStorages = [], purchases = [], payments = [], coldPayments = [] }) => {
  ...
  return (
    <div style={s.content}>
      <div style={{ fontWeight: 800, fontSize: 17, marginBottom: 12, color: clr.accent }}>🚚 Dispatch Value & Settlements</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14 }}>
        ...
        <div style={{ ...s.card2, background: clr.green + "18", border: `1px solid ${clr.green}` }}>
          <div style={s.label}>Total Sale Received</div>
          <div style={{ fontSize: 18, fontWeight: 900, color: clr.green }}>₹{fmt(grandSaleReceived)}</div>
        </div>
      </div>

      <div style={{ ...s.label, fontSize: "13px", color: clr.text, marginBottom: 8, textTransform: "none" }}>🏢 Party-Wise Dispatch Breakdown</div>
      {partyWiseSummary.map(p => (
        <div key={p.partyId} style={{ ...s.card, borderLeft: `4px solid ${clr.purple}` }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, fontSize: 13 }}>
            <div><span style={{ color: clr.muted, fontSize: 11 }}>GP DISPATCH VALUE</span><div style={{ fontWeight: 700, color: clr.blue }}>₹{fmt(p.totalDispatchValue)}</div></div>
            <div><span style={{ color: clr.muted, fontSize: 11 }}>SALE RECEIVED</span><div style={{ fontWeight: 700, color: clr.green }}>₹{fmt(p.totalSaleReceived)}</div></div>
            <div><span style={{ color: clr.muted, fontSize: 11 }}>CASH RECEIVED</span><div>₹{fmt(p.totalPaymentsRec)}</div></div>
            <div><span style={{ color: clr.muted, fontSize: 11 }}>PENDING DUE</span><div style={{ fontWeight: 800, color: p.pendingDue > 0 ? clr.red : clr.green }}>₹{fmt(p.pendingDue)}</div></div>
          </div>
        </div>
      ))}

      <div style={{ ...s.label, fontSize: "13px", color: clr.text, margin: "16px 0 8px 0", textTransform: "none" }}>❄️ Cold Storage Ledger</div>
      {coldWiseSummary.map(c => (
        <div key={c.coldId} style={{ ...s.card, borderLeft: `4px solid ${clr.orange}` }}>
          ...
          <div style={s.divider} />
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, fontSize: 13 }}>
            <div><span style={{ color: clr.muted, fontSize: 11 }}>LOADED VALUE</span><div style={{ fontWeight: 700, color: clr.orange }}>₹{fmt(c.totalColdDispatchValue)}</div></div>
            <div><span style={{ color: clr.muted, fontSize: 11 }}>PAID BACK</span><div style={{ fontWeight: 700, color: clr.green }}>₹{fmt(c.totalPaidBack)}</div></div>
          </div>
        </div>
      ))}
    </div>
...
// ===== DASHBOARD SCREEN =====
const DashboardScreen = ({ purchases = [], dispatches = [], payments = [] }) => {
  ...
  return (
    <div style={s.content}>
      <div style={{ ...s.card, background: dynamicNetProfit >= 0 ? clr.green + "1a" : clr.red + "1a", borderColor: dynamicNetProfit >= 0 ? clr.green : clr.red, marginBottom: 14 }}>
        ...
        <div style={s.rowBetween}>
          <div>
            <div style={{ fontSize: 12, color: clr.muted }}>Net Profit</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14 }}>
        <div style={s.card2}>
          <div style={s.label}>Purchased Bags</div>
          <div style={{ fontSize: 16, fontWeight: 800 }}>{fmt(totalBagsPurchased)}</div>
          <div style={{ fontSize: 11, color: clr.muted }}>In Stock: {fmt(totalBagsRemaining)}</div>
        </div>
        <div style={s.card2}>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <input style={s.input} placeholder="Type Lot ID or Gatepass ID..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
      </div>

      {q !== "" && (
        <div>
          <div style={{ ...s.label, color: clr.accent, marginBottom: 6 }}>Results ({filteredPurchases.length + filteredDispatches.length})</div>
          {filteredPurchases.map(p => (
            <div key={p.id} onClick={() => setSelectedDetail({ type: "purchase", primary: p })} style={{ ...s.card, cursor: "pointer", borderLeft: `4px solid ${clr.blue}` }}>
              <div style={s.rowBetween}><strong>Lot: {p.lot_id}</strong><Badge v={`${p.manual_bags} Bags`} color={clr.blue} /></div>
            </div>
          ))}
          {filteredDispatches.map(d => (
            <div key={d.id} onClick={() => setSelectedDetail({ type: "dispatch", primary: d })} style={{ ...s.card, cursor: "pointer", borderLeft: `4px solid ${clr.purple}` }}>
              <div style={s.rowBetween}><strong>GP: {d.gatepass_id}</strong><Badge v={`${d.vehicle_number}`} color={clr.purple} /></div>
            </div>
          ))}
        </div>
      )}

      <Modal open={!!selectedDetail} onClose={() => setSelectedDetail(null)} title="🔄 Detail View">
        {selectedDetail && (
