...
const s = {
  container: { maxWidth: 480, margin: "0 auto", background: clr.bg, minHeight: "100vh", paddingBottom: 100, fontFamily: "sans-serif", position: "relative" },
  header: { padding: "16px", background: clr.white, borderBottom: `1px solid ${clr.border}`, position: "sticky", top: 0, zIndex: 100 },
  title: { fontSize: 20, fontWeight: "800", color: clr.accent, margin: 0 },
  content: { padding: 16 },
  card: { background: clr.white, borderRadius: 12, padding: 16, marginBottom: 16, border: `1px solid ${clr.border}` },
  grid: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 },
  statLabel: { fontSize: 11, color: clr.muted, fontWeight: "600", textTransform: "uppercase", marginBottom: 4, display: "block" },
  statValue: { fontSize: 16, fontWeight: "700" },
  alert: { padding: 12, borderRadius: 8, marginBottom: 16, fontSize: 12, fontWeight: "600", border: "1px solid" },
...
export default function PotatoERP() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      pendingReceive, advanceReceived, profit, stockMismatch
    };
  ...
  return (
    <div style={s.container}>
      ...
      <main style={s.content}>
          ...
          <>
            {purchases.length === 0 ? (
              <div style={{ ...s.card, textAlign: "center", padding: "40px 0" }}>
                <p style={{ color: clr.muted }}>No Purchases Yet</p>
              </div>
            ) : (
              <>
                {/* 4. Mismatch Alert */}
                {stats.stockMismatch && (
                  <div style={{ ...s.alert, background: "#FEF2F2", color: clr.red, borderColor: "#FEE2E2" }}>
                    ⚠️ Stock Mismatch: Check Purchase vs Dispatch
                  </div>
                )}

                <div style={s.grid}>
                  <div style={s.card}>
                    <span style={s.statLabel}>Total Bought</span>
                    <span style={s.statValue}>{fmt(stats.totalPurchasedBags)} Bags</span>
                  </div>
                  <div style={s.card}>
                    <span style={s.statLabel}>In Stock</span>
                    <span style={{ ...s.statValue, color: clr.blue }}>{fmt(stats.totalRemainingBags)} Bags</span>
                  </div>
                </div>

                <div style={s.grid}>
                  <div style={s.card}>
                    <span style={s.statLabel}>Pending Receive</span>
                    <span style={{ ...s.statValue, color: stats.pendingReceive > 0 ? clr.gold : clr.text }}>
                      ₹{fmt(stats.pendingReceive)}
                    </span>
                  </div>
                  {stats.advanceReceived > 0 ? (
                    <div style={s.card}>
                      <span style={s.statLabel}>Advance Received</span>
                      <span style={{ ...s.statValue, color: clr.green }}>₹{fmt(stats.advanceReceived)}</span>
                    </div>
                  ) : (
                    <div style={s.card}>
                      <span style={s.statLabel}>Bags Sold</span>
                      <span style={s.statValue}>{fmt(stats.totalDispatchedBags)}</span>
                    </div>
                  )}
                </div>
                ...
                <div style={s.card}>
                  <span style={s.statLabel}>
                    {stats.profit > 0 ? "Profit" : stats.profit < 0 ? "Loss" : "Status"}
                  </span>
                  <span style={{ ...s.statValue, color: stats.profit > 0 ? clr.green : stats.profit < 0 ? clr.red : clr.blue }}>
                    {stats.profit === 0 ? "Break-even" : `₹${fmt(Math.abs(stats.profit))}`}
                  </span>
                </div>
              </>
            )}
        ...
        {/* Restore generic view for other tabs to prevent blank screen */}
        {tab !== "dash" && (
          <div style={{ ...s.card, textAlign: "center", padding: 40 }}>
            <p style={{ color: clr.muted }}>{tab.toUpperCase()} Section - Active</p>
          </div>
        )}
      </main>

      {/* 5. Navigation (Removed Supabase Card & Fixed Layout) */}
      <nav style={s.navBar}>
      ...
      {showMore && (
        <div style={{ position: "fixed", bottom: 70, left: "50%", transform: "translateX(-50%)", width: "90%", maxWidth: 400, background: clr.white, borderRadius: 12, boxShadow: "0 10px 25px rgba(0,0,0,0.1)", padding: 8, zIndex: 300, border: `1px solid ${clr.border}` }}>
          <button onClick={() => { setTab("sale"); setShowMore(false); }} style={{ width: "100%", padding: 12, textAlign: "left", border: "none", background: "none", display: "flex", alignItems: "center", gap: 12, borderBottom: `1px solid ${clr.border}` }}>
            <Icon name="sale" color={clr.muted} /> Sale Section
          </button>
          <button onClick={() => { setTab("payment"); setShowMore(false); }} style={{ width: "100%", padding: 12, textAlign: "left", border: "none", background: "none", display: "flex", alignItems: "center", gap: 12 }}>
            <Icon name="payment" color={clr.muted} /> Payments
          </button>
        </div>
      )}
    </div>
