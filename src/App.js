...
// ─── NEW FEATURE: LOT STATUS UTILITY FUNCTION ───────────────────────────────
const getLotStatus = (lot, dispatches = [], sales = []) => {
  const totalDispatched = dispatches
    .flatMap(d => d.items || [])
    .filter(i => i.lotId === lot.lotId)
    .reduce((sum, i) => sum + parseFloat(i.bags || 0), 0);

  const totalSales = sales
    .flatMap(s => s.lotSales || [])
    .filter(ls => ls.lotId === lot.lotId)
    .reduce((sum, ls) => sum + parseFloat(ls.bags || 0), 0);

  const effectiveBags = lot.pricingType === "STD" 
    ? parseFloat(lot.stdBags || 0) 
    : parseFloat(lot.manualBags || 0);
...
// ─── NEW FEATURE: SEARCH MODAL COMPONENT ─────────────────────────────────────
const SearchModal = ({ open, onClose, purchases, dispatches, sales, varieties, gradings, coldStorages, mandis, parties }) => {
  ...
  const searchLotNumber = () => {
    const lot = purchases.find(p => p.lotId?.toLowerCase() === query.toLowerCase().trim());
    if (!lot) return null;
  ...
  const searchVehicle = () => {
    const dispatch = dispatches.find(d => d.vehicleNumber?.toLowerCase() === query.toLowerCase().trim());
  ...
  const searchFarmer = () => {
    const farmerLots = purchases.filter(p => 
      p.kisanName?.toLowerCase().includes(query.toLowerCase().trim())
    );
  ...
  const searchCold = () => {
    const cold = coldStorages.find(c => 
  ...
  const result = results ? 
    (searchLotNumber() || searchVehicle() || searchFarmer() || searchMandi() || searchTrader() || searchCold())
    : null;
    ...
    <Modal open={open} onClose={onClose} title="🔍 खोज">
      <div style={{ marginBottom: 16 }}>
        <input
          type="text"
          style={s.input}
          placeholder="लॉट नंबर, वाहन, किसान, मंडी, व्यापारी..."
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          onKeyPress={(e) => e.key === "Enter" && handleSearch()}
        />
        <button onClick={handleSearch} style={{ ...s.btn(), width: "100%", marginTop: 10 }}>
          <Icon name="search" size={16} color="#000" /> खोजें
        </button>
      </div>
        ...
        <>
          {result.type === "lot" && (
            <div style={s.card}>
              ...
              <div style={{ marginBottom: 10 }}>
                <div style={s.label}>किसान</div>
                <div style={{ fontWeight: 600 }}>{result.lot.kisanName}</div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 }}>
                <div>
                  <div style={s.label}>विविधता</div>
                  <div style={{ fontWeight: 600 }}>{getName(varieties, result.lot.varietyId)}</div>
                </div>
                <div>
                  <div style={s.label}>ग्रेडिंग</div>
                  <div style={{ fontWeight: 600 }}>{getName(gradings, result.lot.gradingId)}</div>
                </div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 10 }}>
                <Stat label="कुल बैग" value={fmt(result.lot.pricingType === "STD" ? result.lot.stdBags : result.lot.manualBags, 1)} color={clr.blue} />
                ...
                <>
                  ...
                  {result.dispatches.map((d, idx) => (
                    <div key={idx} style={s.card2}>
                      ...
                      <div style={{ fontSize: 12, color: clr.muted, marginTop: 4 }}>
                        मंडी: {getName(mandis, d.mandiId)} | बैग: {d.items?.find(i => i.lotId === result.lot.lotId)?.bags || 0}
                      </div>
                    </div>
                  ))}
                </>
          ...
          {result.type === "vehicle" && (
            <div style={s.card}>
              ...
              <div style={s.divider} />
              <div style={{ fontSize: 12, color: clr.muted, marginBottom: 12 }}>
                तारीख: {fmtDate(result.dispatch.date)} | मंडी: {getName(mandis, result.dispatch.mandiId)}
              </div>
              ...
              {result.lots.map((item, idx) => (
                <div key={idx} style={s.card2}>
                  ...
                  <div style={{ fontSize: 12, color: clr.muted, marginTop: 4 }}>
                    बैग: {fmt(item.bags)} | किसान: {item.lot?.kisanName}
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={s.card}>
              ...
              {result.lots.map(({ lot, status }) => (
                <div key={lot.id} style={{ ...s.card2, marginBottom: 8 }}>
                  ...
                  <div style={{ fontSize: 12, color: clr.muted, marginTop: 6 }}>
                    विविधता: {getName(varieties, lot.varietyId)} | वजन: {fmt(lot.totalWeight)} kg
                  </div>
                  <div style={{ fontSize: 12, color: clr.muted }}>
                    कुल: ₹{fmt(lot.totalAmount)} | शेष बैग: {fmt(status.remaining, 1)}
                  </div>
                </div>
              ))}
            </div>
          ...
          {result.type === "trader" && (
            <div style={s.card}>
              ...
              <div style={s.divider} />
              <div style={{ fontSize: 13, fontWeight: 600, color: clr.accent, marginBottom: 8 }}>
                कुल बिक्रय: {result.sales.length} transaction
              </div>
              {result.sales.map((s, idx) => (
                <div key={idx} style={{ ...s.card2, fontSize: 12 }}>
                  <div style={s.rowBetween}>
                    <span>लॉट्स: {s.lotSales?.length || 0}</span>
              ...
              {result.lots.map((lot, idx) => {
                ...
                return (
                  <div key={idx} style={s.card2}>
                    ...
                    <div style={{ fontSize: 12, color: clr.muted, marginTop: 6 }}>
                      किसान: {lot.kisanName} | वजन: {fmt(lot.totalWeight)} kg
                    </div>
                  </div>
                );
              })}
...
const PurchaseScreen = ({ purchases, setPurchases, varieties, gradings, coldStorages, dispatches, setDispatches, sales, setSales, payments, setPayments, mandis, parties }) => {
  ...
  const [showForm, setShowForm] = useState(false);
  const [showSearch, setShowSearch] = useState(false); // NEW STATE FOR SEARCH MODAL
  ...
  return (
    <div>
      {/* UPDATED HEADER AREA TO INCLUDE SEARCH ICON AND COMPONENT */}
      <div style={{ ...s.rowBetween, padding: "12px 16px 0" }}>
        <span style={{ fontWeight: 700, fontSize: 16 }}>🥔 Purchase (खरीद)</span>
        <div style={s.row}>
          <button onClick={() => setShowSearch(true)} style={s.btnSm()}>
            <Icon name="search" size={16} />
          </button>
          <button onClick={() => { setForm(blank); setEditId(null); setErr(""); setShowForm(true); }} style={s.btn()}>
            <Icon name="add" size={16} color="#000" /> New Lot
          </button>
        </div>
      </div>
      ...
      <Modal open={showForm} onClose={() => setShowForm(false)} title={editId ? "Edit Purchase" : "New Purchase"}>
        ...
        <div style={{ ...s.card2, ...s.rowBetween, marginBottom: 12 }}>
          ...
          <span style={{ fontWeight: 800, color: clr.accent, fontSize: 16 }}>₹{fmt(totalAmt)}</span>
        </div>
        <button onClick={save} style={{ ...s.btn(), width: "100%" }}>Save</button>
      </Modal>
    </div>
